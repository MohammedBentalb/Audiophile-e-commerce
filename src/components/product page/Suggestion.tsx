      ...
      {data?.others.map((item, i) =>(
        <div key={i} className='flex w-full max-w-[21.875rem] flex-col items-center gap-10 max-sm:gap-8 '>
          <div className='relative h-[19.875rem] w-full max-w-[21.875rem] rounded-lg bg-primary-lightGray max-md:max-w-[13.9375rem] max-sm:h-[7.5rem] max-sm:max-w-[20.4375rem]'>
            <Image fill alt={`${item.name} audiophile`} src={`${item.image.desktop.substring(1)}`} sizes="" className='rounded-lg object-cover max-md:hidden'/>
            <Image fill alt={`${item.name} audiophile`} src={`${item.image.tablet.substring(1)}`} sizes="" className='rounded-lg object-cover max-sm:hidden md:hidden'/>
            <Image fill alt={`${item.name} audiophile`} src={`${item.image.mobile.substring(1)}`} sizes='' className='rounded-lg object-cover sm:hidden'/>
          </div>
